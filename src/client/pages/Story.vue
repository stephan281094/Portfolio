<template>
  <main class="o-main">
    ...
    <div class="o-content">
      <h1 class="o-content__title">{{ story.title }}</h1>
      <p>
  ...
  export default {
    ...
    metaInfo () {
      return {
        ...
        meta: [
          { name: 'description', content: this.story.description }
        ]
      }
    },
    data () {
      return {
        story: this.$store.state.stories.find((story) => {
          return story.slug === this.$route.params.slug
        })
      }
    },
    computed: {
      breadcrumbs () {
        return [
          ...
          { name: this.story ? this.story.title : '...' },
        ]
      }
    },
    components: {
